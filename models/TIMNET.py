...
class TemporalAwareBlock(nn.Module):
    def __init__(self, nb_filters, kernel_size, dilation_rate, dropout_rate, activation):
        super(TemporalAwareBlock, self).__init__()
        self.conv1 = CausalConv1d(nb_filters, nb_filters, kernel_size, dilation=dilation_rate)
        self.bn1 = nn.LazyBatchNorm1d()
        self.activation = getattr(F, activation)
        self.dropout1 = SpatialDropout1d(dropout_rate)

        self.conv2 = CausalConv1d(nb_filters, nb_filters, kernel_size, dilation=dilation_rate)
...
class TIMNET(nn.Module):
    def __init__(self, nb_filters=64, kernel_size=2, nb_stacks=1, dilations=None, activation="relu", dropout_rate=0.1, return_sequences=True):
        super(TIMNET, self).__init__()
        self.return_sequences = return_sequences
        self.activation = activation
        self.dropout_rate = dropout_rate
        self.dilations = dilations if dilations is not None else [2 ** i for i in range(8)]
        self.nb_stacks = nb_stacks
        self.kernel_size = kernel_size
        self.nb_filters = nb_filters

        self.conv_forward = CausalConv1d(self.nb_filters, self.nb_filters, kernel_size=1, dilation=1)
        self.conv_backward = CausalConv1d(self.nb_filters, self.nb_filters, kernel_size=1, dilation=1)
        ...
        for s in range(self.nb_stacks):
            for dilation in self.dilations:
                self.temporal_blocks_forward.append(TemporalAwareBlock(nb_filters, kernel_size, dilation, dropout_rate, activation))
                self.temporal_blocks_backward.append(TemporalAwareBlock(nb_filters, kernel_size, dilation, dropout_rate, activation))

        self.global_avg_pool = nn.AdaptiveAvgPool1d(1)
    def build(self, input):
        print("aaaa", input.shape)
        self.conv_forward = CausalConv1d(input.shape[1], self.nb_filters, kernel_size=1, dilation=1).to(input.device)
        self.conv_backward = CausalConv1d(input.shape[1], self.nb_filters, kernel_size=1, dilation=1).to(input.device)
    ...
    def forward(self, inputs):
        ...
        for i, (block_forward, block_backward) in enumerate(zip(self.temporal_blocks_forward, self.temporal_blocks_backward)):
            ...
            temp_skip = self.global_avg_pool(temp_skip.permute(0,2,1)).view(inputs.size(0),1,-1) # b, 1, nb_filters
