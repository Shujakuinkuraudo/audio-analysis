...
class Train_process:
    ...
    def test_fold(
        ...
        accs = []
        max_accs = []
        for i, (train_dataset, test_dataset) in (
            tq := tqdm.tqdm(enumerate(fold), total=len(fold))
        ):
            ...
            optimizer = optimizer_cls(model.parameters(), lr=1e-3, betas=(0.93, 0.98))
            lr_scheduler = torch.optim.lr_scheduler.StepLR(
                optimizer,
                step_size=run.config["lr_step_size"],
                gamma=run.config["lr_step_gamma"],
            )
            max_acc = 0
            ...
            for epoch in (epoch_tq := tqdm.tqdm(range(epochs))):
                ...
                for (
                    ...
                    target = 0.9 * target + 0.1 / len(labels)
                    loss = model.loss_function(output, target)
                    loss.backward()
                    optimizer.step()
                if run:
                    run.log(
                        {f"{name}_{i}_epoch": epoch, f"{name}_{i}_loss": loss.item()}
                    )
                ...
                if run:
                    run.log(
                        {
                            f"{name}_{i}_epoch": epoch,
                            f"{name}_{i}_acc": now_acc,
                            f"{name}_{i}_max_acc": max_acc,
                        }
                    )
                epoch_tq.set_description(
                    f"Fold {i} epoch {epoch} accuracy: {now_acc} loss: {loss.item()}"
                )
