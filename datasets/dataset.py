...
class dataset:
    def get_feature(self, wave_form: torch.Tensor, sr) -> Tuple[torch.Tensor]:
        ...
        mfcc_transform = torchaudio.transforms.MFCC(sample_rate=sr, n_mfcc=39)
        mfcc_total = mfcc_transform(wave_form).T  # 1, 13, 201
    ...
    def get_feature_data(self):
        ...
        for zcr, energy, mfcc_total, max_val, fft, mfcc_partial, target in tqdm.tqdm(
            self
        ):
