...
def emodb_fold_dl(
    ...
    each_peole = 10 // fold
    leave_out_peole = [
        [j + i for i in range(each_peole)]
        for j in range(0, 10 - each_peole + 1, each_peole)
    ]
    return [
        [
            emodb_dataset(
                root,
                train=True,
                leave_out_people_id=leave_out_peole[i],
                sr=sr,
                win_length=win_length,
                hop_length=hop_length,
                n_fft=n_fft,
...
def emodb_fold_ml(root: str = "data/emodb", fold: int = 5, sr=16000):
    ...
    return [
        [
            emodb_dataset(
                root, train=True, leave_out_people_id=leave_out_peole[i], sr=sr
            ).get_feature_data(),
            emodb_dataset(
                root, train=False, leave_out_people_id=leave_out_peole[i], sr=sr
            ).get_feature_data(),
        ]
        for i in range(fold)
    ], emodb_dataset.emotions
